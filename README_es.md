...
# actualización-opkg
Enumere e instale los paquetes actualizables de OpenWRT/LEDE opkg.  

Pequeña aplicación `ash` para facilitar las actualizaciones del paquete opkg.

**¡Deberías comprobar si hay conflictos de configuración después de las actualizaciones!**  
**¡Asegúrate de tener suficiente espacio en la raíz antes de instalar cosas!**  
¡Este script es lo suficientemente pequeño pero el soporte SSL para curl/wget no lo es!  
¡También necesita espacio libre para descargar e instalar los paquetes!  
  
**Si usas el tronco OpenWRT (instantáneas de desarrollo), probablemente NO deberías actualizar**  
Actualizar las versiones de desarrollo puede bloquear su dispositivo.  
Debería intentar hacer una copia de seguridad de su configuración y realizar una instalación completa de una instantánea posterior si está ejecutando dev/trunk.  
Las versiones principales están bien (solo las troncales/instantáneas se ven afectadas por esto).  
Probablemente sea lo contrario para las versiones (se recomienda actualizar), ya que obtendrá parches de seguridad si los actualiza.
...
```
## Instalar usando `git`:
Clónelo en el directorio actual y use la opción `-i` ​​para instalarlo en `/usr/sbin/opkg-upgrade`
```
...
## Instalar usando `curl` o `wget`:

#### NOTA: curl/wget puede fallar debido a que faltan certificados SSL.
Puede optar por ignorar la verificación de certificados usando:
 - `curl -k`
 - `wget --no-check-certificate`  

O necesitarás arreglar tu instalación `/etc/ssl/certs/ca-certificates.crt`.  
Tenga en cuenta que la compatibilidad con SSL requiere bastante espacio de almacenamiento.  

Esto debería ser suficiente para que SSL funcione:  
```
opkg install ca-certificates openssl-util
```
Y esta puede ser una solución si aún tienes problemas:
```
...
### Oneliners para ejecutar desde Internet (descargas a `/tmp`):
```bash
# using wget with SSL
...
### Instalación local en el directorio actual (`./opkg-upgrade.sh`):
Ejecutar con `./opkg-upgrade.sh` después de la descarga
```bash
# using wget with SSL
...
### Instalación del sistema en `/usr/sbin/opkg-upgrade` (sin extensión .sh):
Ejecutar con `opkg-upgrade` después de la descarga 
```bash
# using wget with SSL
