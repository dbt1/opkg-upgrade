...
# opkg-upgrade
Listen und installieren Sie aktualisierbare OpenWRT/LEDE opkg-Pakete.  

Kleine `ash`-App für einfachere OPKG-Paket-Upgrades.

**Sie sollten nach Upgrades auf Konfigurationskonflikte prüfen!**  
**Stellen Sie sicher, dass Sie über genügend Speicherplatz im Root verfügen, bevor Sie Dinge installieren!**  
Dieses Skript ist klein genug, aber die SSL-Unterstützung für curl/wget ist es nicht!  
Sie benötigen außerdem freien Speicherplatz zum Herunterladen und Installieren der Pakete!  
  
**Wenn Sie OpenWRT-Trunk (Entwickler-Snapshots) verwenden, sollten Sie wahrscheinlich KEIN Upgrade durchführen**  
Durch die Aktualisierung von Entwicklungsversionen kann Ihr Gerät mit einem Soft-Brick versehen werden.  
Sie sollten versuchen, Ihre Konfiguration zu sichern und eine vollständige Installation eines späteren Snapshots durchzuführen, wenn Sie dev/trunk ausführen.  
Hauptversionen sind in Ordnung (davon sind nur Trunk/Snapshots betroffen).  
Bei Releases ist es wahrscheinlich umgekehrt (ein Upgrade wird empfohlen), da Sie Sicherheitspatches erhalten, wenn Sie diese aktualisieren.
...
### Wenn alles auf dem neuesten Stand ist:
```
# ./opkg-upgrade.sh -f -n
...
Klonen Sie es im aktuellen Verzeichnis und verwenden Sie die Option `-i`, um es unter `/usr/sbin/opkg-upgrade` zu installieren.
```
...
## Installieren Sie mit „curl“ oder „wget“:

#### HINWEIS: Curl/wget schlägt möglicherweise aufgrund fehlender SSL-Zertifikate fehl.
Sie können die Zertifikatsprüfung auch ignorieren, indem Sie Folgendes tun:
 - `curl -k`
 - `wget --no-check-certificate`  

Oder Sie müssen Ihre `/etc/ssl/certs/ca-certificates.crt`-Installation reparieren.  
Bitte beachten Sie, dass die SSL-Unterstützung recht viel Speicherplatz beansprucht.  

Das sollte ausreichen, damit SSL funktioniert:  
```
opkg install ca-certificates openssl-util
```
Und dies könnte eine Lösung sein, wenn Sie immer noch Probleme haben:
```
...
### Oneliner zum Ausführen aus dem Internet (Downloads nach „/tmp“):
```bash
# using wget with SSL
...
### Lokale Installation im aktuellen Verzeichnis (`./opkg-upgrade.sh`):
Nach dem Herunterladen mit `./opkg-upgrade.sh` ausführen
```bash
# using wget with SSL
...
### Systeminstallation nach „/usr/sbin/opkg-upgrade“ (keine .sh-Erweiterung):
Nach dem Herunterladen mit `opkg-upgrade` ausführen 
```bash
# using wget with SSL
