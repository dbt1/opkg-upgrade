...
# opkg-aggiornamento
Elenca e installa i pacchetti aggiornabili OpenWRT/LEDE opkg.  

Piccola app `ash` per aggiornamenti più semplici del pacchetto opkg.

**Dovresti verificare la presenza di conflitti di configurazione dopo gli aggiornamenti!**  
**Assicurati di avere abbastanza spazio su root prima di installare qualcosa!**  
Questo script è abbastanza piccolo ma il supporto SSL per curl/wget non lo è!  
Hai anche bisogno di spazio libero per scaricare e installare i pacchetti!  
  
**Se utilizzi il trunk OpenWRT (istantanee di sviluppo), probabilmente NON dovresti eseguire l'aggiornamento**  
L'aggiornamento delle versioni di sviluppo può rendere il tuo dispositivo soft-brick.  
Dovresti provare a eseguire il backup della configurazione ed eseguire un'installazione completa di uno snapshot successivo se stai utilizzando dev/trunk.  
Le versioni principali vanno bene (sono interessati solo i trunk/le istantanee).  
Probabilmente è il contrario per le versioni (consigliato per l'aggiornamento), poiché otterrai patch di sicurezza se le aggiorni.
...
```
## Installa utilizzando `git`:
Clonalo nella directory corrente e utilizza l'opzione `-i` ​​per installarlo su `/usr/sbin/opkg-upgrade`
```
...
## Installa utilizzando `curl` o `wget`:

#### NOTA: curl/wget potrebbe non riuscire a causa della mancanza di certificati SSL.
Puoi scegliere di ignorare il controllo dei certificati utilizzando:
 - `curl -k`
 - `wget --no-check-certificate`  

Oppure dovrai correggere l'installazione di `/etc/ssl/certs/ca-certificates.crt`.  
Tieni presente che il supporto SSL richiede molto spazio di archiviazione.  

Questo dovrebbe essere sufficiente per far funzionare SSL:  
```
opkg install ca-certificates openssl-util
```
E questa potrebbe essere una soluzione alternativa se hai ancora problemi:
```
...
### Oneliner da eseguire da Internet (scarica in `/tmp`):
```bash
# using wget with SSL
...
### Installazione locale nella directory corrente ( `./opkg-upgrade.sh` ):
Esegui con `./opkg-upgrade.sh` dopo il download
```bash
# using wget with SSL
...
### Installazione del sistema su `/usr/sbin/opkg-upgrade` (nessuna estensione .sh):
Esegui con `opkg-upgrade` dopo il download 
```bash
# using wget with SSL
