...
# opkg-mise à niveau
Répertoriez et installez les packages évolutifs OpenWRT / LEDE opkg.  

Petite application `ash` pour des mises à niveau plus faciles du package opkg.

**Vous devriez vérifier les conflits de configuration après les mises à niveau !**  
**Assurez-vous d'avoir suffisamment d'espace sur root avant d'installer des éléments !**  
Ce script est assez petit mais la prise en charge SSL pour curl/wget ne l'est pas !  
Vous avez également besoin d'espace libre pour télécharger et installer les packages !  
  
**Si vous utilisez le tronc OpenWRT (instantanés de développement), vous ne devriez probablement PAS mettre à niveau**  
La mise à niveau des versions de développement peut rendre votre appareil soft-brick.  
Vous devriez essayer de sauvegarder votre configuration et d'effectuer une installation complète d'un instantané ultérieur si vous exécutez dev/trunk.  
Les versions principales sont correctes (seuls les troncs/instantanés sont affectés par cela).  
C'est probablement le contraire pour les versions (mise à niveau recommandée), puisque vous obtiendrez des correctifs de sécurité si vous les mettez à niveau.
...
```
## Installez en utilisant `git` :
Clonez-le dans le répertoire actuel et utilisez l'option `-i` ​​pour l'installer sur `/usr/sbin/opkg-upgrade`
```
# git clone https://github.com/dbt1/opkg-upgrade.git
# cd opkg-upgrade
# ./opkg-upgrade.sh -i
```

## Installez en utilisant `curl` ou `wget` :

#### REMARQUE : curl / wget peut échouer en raison de certificats SSL manquants.
Vous pouvez choisir d'ignorer la vérification des certificats en utilisant :
 - `curl -k`
 - `wget --no-check-certificate`  

Ou vous devrez corriger votre installation `/etc/ssl/certs/ca-certificates.crt`.  
Veuillez noter que la prise en charge SSL prend beaucoup d'espace de stockage.  

Cela devrait suffire pour faire fonctionner SSL :  
```
opkg install ca-certificates openssl-util
```
Et ceci peut être une solution de contournement si vous rencontrez toujours des problèmes :
```
...
### Oneliners à exécuter depuis Internet (téléchargements sur `/tmp`) :
```bash
# using wget with SSL
wget 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -O "/tmp/opkg-upgrade.sh" && chmod 755 "/tmp/opkg-upgrade.sh" && /tmp/opkg-upgrade.sh

# using wget WITHOUT SSL
wget --no-check-certificate 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -O "/tmp/opkg-upgrade.sh" && chmod 755 "/tmp/opkg-upgrade.sh" && /tmp/opkg-upgrade.sh

# using curl with SSL
curl -L 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -o "/tmp/opkg-upgrade.sh" && chmod 755 "/tmp/opkg-upgrade.sh" && /tmp/opkg-upgrade.sh

# using curl WITHOUT SSL
curl -k -L 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -o "/tmp/opkg-upgrade.sh" && chmod 755 "/tmp/opkg-upgrade.sh" && /tmp/opkg-upgrade.sh
```

### Installation locale dans le répertoire actuel ( `./opkg-upgrade.sh` ) :
Exécutez avec `./opkg-upgrade.sh` après le téléchargement
```bash
# using wget with SSL
wget 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -O "opkg-upgrade.sh" && chmod 755 "opkg-upgrade.sh"

# using wget WITHOUT SSL
wget --no-check-certificate 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -O "opkg-upgrade.sh" && chmod 755 "opkg-upgrade.sh"

# using curl with SSL
curl -L 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -o "opkg-upgrade.sh" && chmod 755 "opkg-upgrade.sh"

# using curl WITHOUT SSL
curl -k -L 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -o "opkg-upgrade.sh" && chmod 755 "opkg-upgrade.sh"
```

### Installation du système sur `/usr/sbin/opkg-upgrade` (pas d'extension .sh) :
Exécutez avec `opkg-upgrade` après le téléchargement 
```bash
# using wget with SSL
wget 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -O "/usr/sbin/opkg-upgrade" && chmod 755 "/usr/sbin/opkg-upgrade"

# using wget WITHOUT SSL
wget --no-check-certificate 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -O "/usr/sbin/opkg-upgrade" && chmod 755 "/usr/sbin/opkg-upgrade"

# using curl with SSL
curl -L 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -o "/usr/sbin/opkg-upgrade" && chmod 755 "/usr/sbin/opkg-upgrade"

# using curl WITHOUT SSL
curl -k -L 'https://raw.githubusercontent.com/dbt1/opkg-upgrade/refs/heads/master/opkg-upgrade.sh' -o "/usr/sbin/opkg-upgrade" && chmod 755 "/usr/sbin/opkg-upgrade"
```
